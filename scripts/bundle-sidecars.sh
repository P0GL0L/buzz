...
SIDECARS=(buzz-acp buzz-cursor-acp buzz-agent buzz-dev-mcp git-credential-nostr buzz)
...
if [[ ${#missing[@]} -gt 0 ]]; then
    ...
    echo "Run 'cargo build --release -p buzz-acp -p buzz-cursor-acp -p buzz-agent -p buzz-dev-mcp -p git-credential-nostr -p buzz-cli' first." >&2
    exit 1
fi
