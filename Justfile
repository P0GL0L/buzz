...
_ensure-sidecar-stubs:
    ...
    for bin in buzz-acp buzz-antigravity-acp buzz-agent buzz-dev-mcp git-credential-nostr buzz; do
...
dev *ARGS: bootstrap _ensure-sidecar-stubs _ensure-migrations
    ...
    cargo build -p buzz-acp -p buzz-antigravity-acp -p buzz-agent -p buzz-dev-mcp -p buzz-cli -p git-credential-nostr -p buzz-relay
    if [[ -n "{{mesh}}" ]]; then
        export MESH_LLM_NATIVE_RUNTIME_CACHE_DIR="$(./scripts/ensure-mesh-native-runtime.sh)"
    fi
...
desktop-standalone *ARGS: _ensure-sidecar-stubs
    ...
    set -euo pipefail
    export PATH="{{justfile_directory()}}/bin:$PATH"
    cargo build -p buzz-acp -p buzz-antigravity-acp -p buzz-agent -p buzz-dev-mcp -p buzz-cli -p git-credential-nostr
    TARGET=$(rustc -vV | sed -n 's|host: ||p')
...
staging *ARGS: bootstrap _ensure-sidecar-stubs
    ...
    cargo build --release -p buzz-acp -p buzz-antigravity-acp -p buzz-agent -p buzz-dev-mcp -p buzz-cli -p git-credential-nostr
    FEATURES=()
    if [[ -n "{{mesh}}" ]]; then
